...
| KB-P6 | 外部素材获取（yt-dlp 链接下载入库：视频 / 纯音频 + 平台字幕，默认关闭） | ✅ **已完成**（`fetch_media` builtin 工具 + `media_fetch_tasks.media_fetch_task` Celery 任务 + `MediaFetchService.import_document`；提取器白名单 `youtube/bilibili/vimeo/dailymotion/twitch`；admin 公共 AI 配置 `media_fetch` 门控默认关闭；L1 字幕优先回退 ASR；无新表/迁移。**封面未接入**。见 [modules/02-knowledge-base.md §11.18](./modules/02-knowledge-base.md#1118-外部素材获取kb-p6已落地) 与 [knowledge-base-product-form-design.md §5.3](./knowledge-base-product-form-design.md#53-素材获取外部媒体平台下载yt-dlp已落地kb-p6)） |
