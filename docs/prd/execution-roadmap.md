...
>   `api/Dockerfile.render`、`ui/Dockerfile`、`ui/Dockerfile.dev`。
>   UI 的**产物阶段**仍是 `nginx:1.30-alpine`，那是 nginx 不是 node，与本次统一无关。
>   （`docker-compose.dev.yaml` 已随 UI 开发模式统一清理，不再使用，见 `docker/README.md`。）
>
> **部署时的两条硬约束（现状已核实，勿踩）**：
...
| **UX-8 商店预览模式** | router/index.ts, StoreMcpView.vue, StoreSkillsView.vue, StoreToolsView.vue, StoreMcpView.spec.ts（新建）, StoreSkillsView.spec.ts（新建）, StoreToolsView.spec.ts（新建）, execution-roadmap.md | ✅ 已完成 | 资源运营 MCP/Skills/Tools 三个商店页右上角新增「预览商店效果」按钮（storeOps.previewStore 键已有），`window.open` 打开**新增的用户端商店路由** `/store/{mcp,skills,tools}`（复用 store ListView 用户态渲染，与 `/store/public-apps` 同级）；公共应用商店预览此前已实现。**Workflows 边界**：无用户端工作流商店列表页（仅 /admin/store/workflows/:id/preview 是 admin 预览），本次不提供预览按钮，待用户端工作流商店落地后补齐。TDD：3 个 wrapper spec（按钮存在 + window.open 断言）+ parity 全绿 |
| **UX-9 Agent 治理域 UI 翻新 + 排障 + 权限语义化** | AgentPoolView.vue, sub-pool-definition/index.vue, agents/ListView.vue, agents/ChatView.vue, utils/admin-agent-display.ts（新建）, agents.ts（zh/en）, admin_routes_7.py, admin_agent_schema.py, admin_agent_service.py, test_admin_agent_schedule_routes.py, test_admin_agent_routes.py, test_admin_agent_service.py, docs/api/admin-agents-api.md | ✅ 已完成 | **排障**：`admin_routes_7.py` 两处 `except FailException` 漏导入，导致 Agent 定时任务填错 cron 时本该 400 的校验错误升级为 500（`NameError`）；补函数内延迟导入（遵循该文件规避循环依赖的既有约定）+ 反向测试锁定。**UI 翻新**：Agent 池配置页与子池定义页原是手写原生 `<table>` 裸骨架，管理端 Agent 列表/对话页为朴素 Tailwind 拼装；统一到 ToolsView 确立的「KPI 卡 + 精修表格 + 语义 tag + 确定性渐变头像」语言，抽取共享展示工具 `admin-agent-display.ts`。**权限语义化（消除两套展示）**：此前 Agent 授权选择器/列表只显示裸权限码（`app:read`），而「角色权限」页已语义化显示中文名——根因是 `/admin/agents/assignable-permissions` 只返回 `codes` 裸码。修法**不是**让 Agent 页改调全量 `/admin/permissions`（那会绕过 `ASSIGNABLE_PERMISSIONS` fail-closed 安全交集，把不可下放权限点暴露到选择器），而是**在同一端点补 `permissions` 语义明细**（与 `codes` 同源同序，名称取自 `PERMISSION_CATALOG` 单一事实源）；前端复用角色页的 `admin.roles.resources.*` 分组文案，选择器按 resource 分组、表格显示中文名（tooltip 保留裸码），**不新增第二份名称映射**。验证：后端 52 测试、前端 admin 全量 24 文件/125 测试（含新建 ListView 语义化契约测试）、parity、ESLint 零告警、`vue-tsc` 唯一错误数与基线持平（42→42，零新增）；端到端实测 `codes=85 / permissions=85` 且同源同序。**设计决策沉淀**：本次曾重新推导出「封禁表今天冗余、可去掉并改由前端过滤」的错误结论，已在 `docs/rbac.md §9.2.1` 记录「为何必须保留」（惰性权限→未来激活→无人值守自主运行提权 + 前端副本无法 fail-closed + viewer 无 permission:read），并补 `TestBannedLayerDefense` 三条守卫测试（含反向验证：移除封禁分支时测试失败），避免后续再次重复推导 |
...
| **UX-8 商店预览模式** | PRI3 | ✅ 已完成 | 见「已完成（UX 快速修复）」表中 UX-8 条目 |
