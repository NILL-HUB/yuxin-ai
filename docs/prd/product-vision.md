...
| 12 | 我的应用列表 | 🎭 **壳子** | `my-apps/ListView.vue` 整页 mock 假数据，真实接口 `listMyApps` 已存在未接 |
...
> 下列 §4.1 ~ §4.3 已于本轮修复并端到端验证；§4.4 仍待处理。
...
### 4.4 前端两处 mock 冒充真实功能（⚠️ 待处理）

- [my-apps/ListView.vue](file:///d:/DEMO/openagent-main/ui/src/views/space/my-apps/ListView.vue#L41-L68)：整页 9 条硬编码假应用，`loadApps` 只 `setTimeout` 回填；点击「打开」用假 id 调接口必 404。真实接口 `GET /my/apps` 已实现。
- [ExternalDataSourceModal.vue](file:///d:/DEMO/openagent-main/ui/src/views/space/datasets/components/ExternalDataSourceModal.vue#L28-L59)：新建/同步/解绑全走 `setTimeout` 假装成功，刷新即丢。真实的 `/external-data-sources` 接口与页面均已存在。
...
| 阶段 | 目标 | 关键交付 |
|---|---|---|
| **P1 底座夯实**（当前） | 让已有能力**真正可用**，消灭壳子与断链 | ✅ 设备 token 链路；✅ AI 计费；✅ cua-driver 打包托管；⏳ 接通 my-apps / 外部数据源 |
...
4. 🔴 前端 my-apps / 外部数据源接通真实接口（§4.4）——消灭最显眼的壳子（待处理）
5. 🟠 工作流商店预览路由修复（§三-14）
6. 🟡 首页推荐池真实化（§三-23）
7. 🟡 Studio 占位页处理：要么实现，要么移除侧边栏入口
