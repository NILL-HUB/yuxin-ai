...
> **v7.0 定位变更（2026-09-11）**：产品形态正式确立为**面向「钰字辈」合伙人的线上生态赋能系统**，对应新时代互联网形态「**万物智联**」（设备听你一句话就自己动，App 收敛为单一 AI 入口）。系统品牌名为 **钰见我**，前期以 AI 助理身份 **小钰** 呈现。v5.2 及以下章节描述的是当前实现底座；「通用 Agent 调度平台」为阶段性技术底座表述，不再代表产品定位。

---

本文档定义 钰见我 的目标架构与核心模块设计方案。系统定位为**面向钰字辈合伙人的线上生态赋能系统**：以 AI 助理「小钰」为统一入口，赋能合伙人的设备、工作与流量获取，并通过合伙人分身、分销体系与内容板块形成线上生态闭环。

> **定位演进关系**：早期表述「通用 Agent 调度平台」（管理员配置能力、用户自然语言提需求、系统自动选模型/Agent/工具）是达成上述定位的**技术底座实现路径**，而非产品目标本身。产品目标以本节的生态赋能定位为准。

---

## 1. 产品形态与生态蓝图（v7.0）

> 本节为产品形态的权威表述（源自招商演讲稿），并对照当前代码给出真实落地状态。**任何与本表冲突的旧描述，以本节为准。**

### 1.1 形态定位

- **新时代互联网形态：万物智联**。区别于「万物互联」（设备能联网），万物智联是设备听一句话就自己动；App 收敛为单一 AI 入口，谁握入口谁握流量。
- **系统形态：面向「钰字辈」合伙人的线上生态赋能系统**。前期以 AI 助理「**小钰**」的身份呈现（品牌名「钰见我」，身份认知「小钰」）。
- **核心目的**：我一个人跑断腿帮不了几个人；有了小钰，她能解决所有人电脑上 90% 的问题，并解决工作痛点与流量获取难题。
...
| 9 | **手机端** | 逐步推出；电脑不关机即可随时随地干活看结果 | 未实现（Capacitor 壳已就绪） | `mobile/` |
| 10 | **内容板块** | 合伙人发图文、上新品、展示内容（类小红书） | ❌ 未实现 | `04-social-creator.md` |
| 11 | **生活服务接入** | 叫滴滴、点外卖、下单购物（美团/高德接口） | 部分：高德已实现；滴滴/美团未实现 | `providers/gaode/` |
| 12 | **AI + 硬件** | 接入 AI 手表/空调/热水器；硬件供应商成为合伙人进入供应链矩阵 | ❌ 未实现（远期） | — |
| 13 | **生态闭环** | 线下供应链矩阵 + 线上 AI 生态与用户资产；小钰成为全国流量入口 | 战略目标（非功能） | — |
| 14 | **私域直播联动** | 存量业务聚焦私域直播，与小钰联动方案设计中 | 方案中 | — |
...
钰见我 当前已经具备 Agent 平台的多个基础模块：主入口 Assistant Agent、公共 Agent A2A 路由、MCP Provider、AppConfig 模型配置、AppConfig 工具绑定、配置中心、后台管理、应用分配、SSE 流式事件、深度思考 Agent 等。这些能力说明系统已经不是一个简单聊天应用，而是具备 Agent 平台底座。
...
让 AI 助理「小钰」成为面向「钰字辈」合伙人的线上生态入口：解决合伙人电脑上 90% 的问题、工作痛点与流量获取难题，并通过合伙人分身、分销体系与内容板块形成生态闭环，最终指向「万物智联」——设备听一句话就自己动，小钰成为流量入口。
...
| 我的 AI | `/my-ai`、`/my/apps` | 普通用户显式使用本人从应用商店添加（fork）的应用；`GET /my/apps` 仅返回 `status=published` 的应用（草稿不上架）。管理员分配应用功能已下线 |
...
| 缺少日常生活类工具 | 20 个内置工具偏查询/生成 | 无法覆盖全场景 | ✅ 已对齐 Hermes：工具池扩展至搜索/网页提取/视频分析/X 搜索/文件搜索与分页读/浏览器补充动作等（邮件/回收站/OS 自动化此前已加），并全量移植 71 个社区 skills 与 8 个主流 MCP server |
...
钰见我 已具备成熟的 Agent 应用平台底座，池治理改造不应推倒重来，而应区分"可复用"与"需重组"两类。
...
>
> **v7.2 执行链路（P1b）**：在授权内核之上装配能力层（L2）与执行层（L1）——`AdminAgentPrincipal` 作为显式入参全链路传递；板块工具内部按 `action` 分支，且**每个 action 显式声明所需权限点（未声明即拒绝）**；写操作按 `automation_policy` 分流（`supervised` 产变更草稿、`autonomous` 直接执行、`blocked` 熔断）；审计新增 `actor_type`（`human`/`agent`）与 `agent_id`。当前只登记 `builtin_tool` 一个样板板块，其余板块按同一模式增量登记。HTTP 入口见 [管理端 Agent API](../api/admin-agents-api.md)。对话式入口与会话表已于 **P2** 落地（`POST /admin/agents/<id>/chat` + 独立会话/消息表）。
>
> **v7.3 记忆治理与运行约束（P4）**：四项收口——① **记忆 GDPR 删除入口** `POST /admin/memory/gdpr-delete`（主体分解入参，服务端构造 `MemoryOwnerKey` 后交 `MemoryGovernor.gdpr_delete`，不信任客户端裸 key）；② **预算闸门** `AdminAgentBudgetGate` 三入口施加（`invoke` / `chat` / admin 定时任务执行），`budget_config` 空则恒放行、Redis 不可用 fail-open；③ **定时任务 admin_agent 通道**（`schedule_task.admin_agent_id` + `task_type='admin_agent_execution'` → `schedule_execution_service` admin 分支 → `AdminAgentExecutionService.run`）；④ **管理端前端对话页 + 记忆面板**（`ui/src/views/admin/agents/*`）。交付与验证见 [execution-roadmap.md ADMIN-P4 节](./execution-roadmap.md#admin-p4-记忆治理与对话入口2026-09-20-完成)，API 契约见 [管理端 Agent API §7–§9](../api/admin-agents-api.md)。
