...
> **v6.0 定位变更**：项目已更名为 **钰心AI**。v5.2 及以下章节描述的是当前实现底座；产品定位正在从“通用 Agent 调度平台”升级为“设备 Agent + 合伙人共创生态”，新架构设计将另行成文并在此索引。

---

本文档定义 钰心AI 的目标架构与核心模块设计方案。系统应从"可配置 Agent 应用平台"演进为"通用 Agent 调度平台"：管理员配置能力，用户自然语言提出需求，系统自动选择模型、Agent 和工具完成任务。
...
钰心AI 当前已经具备 Agent 平台的多个基础模块：主入口 Assistant Agent、公共 Agent A2A 路由、MCP Provider、AppConfig 模型配置、AppConfig 工具绑定、配置中心、后台管理、应用分配、SSE 流式事件、深度思考 Agent 等。这些能力说明系统已经不是一个简单聊天应用，而是具备 Agent 平台底座。
...
把 钰心AI 从"可配置 Agent 应用平台"升级为"通用 Agent 调度平台"：管理员配置能力，用户自然语言提出需求，系统自动选择合适模型、合适 Agent 和合适工具完成任务，并以统一、可靠、成本可控的方式返回结果。
...
钰心AI 已具备成熟的 Agent 应用平台底座，池治理改造不应推倒重来，而应区分"可复用"与"需重组"两类。
