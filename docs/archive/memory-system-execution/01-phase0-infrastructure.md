> **归档状态**：✅ 这系列为记忆系统执行历史快照（2026-09-05 归档），规划已全部落地。请以 docs/prd/memory-system/ 主文档为准。
# Phase 0：基础设施 -- 任务执行文档
...
services:
  ...
  llmops-minio:
    ...
    environment:
      MINIO_ROOT_USER: yujianwo
      MINIO_ROOT_PASSWORD: yujianwo123
    healthcheck:
...
2. **新增 `llmops-neo4j` 服务**（追加到 `services:` 下）：
   ...
   - `environment`：
     - `NEO4J_AUTH: neo4j/yujianwo`（用户名 `neo4j`，密码 `yujianwo`）
...
4. **更新 `llmops-api` 依赖**（可选但推荐）：
   ...
   - 在 `llmops-api` 的 `environment` 下追加（供 I2 配置类读取）：
     ...
     - `NEO4J_USER: neo4j`
     - `NEO4J_PASSWORD: yujianwo`
     - `MINIO_ENDPOINT: llmops-minio:9000`
     - `MINIO_ACCESS_KEY: yujianwo`
     - `MINIO_SECRET_KEY: yujianwo123`
...
- [ ] 浏览器访问 `http://localhost:7474` 可见 Neo4j Browser，用 `neo4j/yujianwo` 可登录。
...
2. **定义各子配置类**（每个类为 `BaseModel`，字段带默认值；环境变量通过 `Settings` 的 `env_prefix` + 嵌套分隔符映射，或子类字段直接用 `validation_alias` 绑定具体环境变量名）。各子类字段定义如下：
   ...
   - **Neo4jConfig**：
     ...
     class Neo4jConfig(BaseModel):
         ...
         password: str = Field(default="yujianwo", validation_alias="NEO4J_PASSWORD")
     ```
...
2. **编写文件头注释**：说明用途、Neo4j 版本要求（5.x）、执行方式（`cat neo4j_init.cypher | cypher-shell -u neo4j -p yujianwo` 或通过 Neo4j Browser 粘贴执行）。
...
- [ ] 文件可被 `cypher-shell` 完整执行无报错：`cat api/internal/migration/neo4j_init.cypher | docker exec -i llmops-neo4j cypher-shell -u neo4j -p yujianwo`。
...
DATABASE_URL = os.getenv("SQLALCHEMY_DATABASE_URI", "postgresql+asyncpg://yujianwo:yujianwo@llmops-db:5432/yujianwo")
...
| `NEO4J_PASSWORD` | `yujianwo` | I1/I2 | Neo4j 密码 |
| `MINIO_ENDPOINT` | `localhost:9000` | I1 | MinIO S3 endpoint |
| `MINIO_ACCESS_KEY` | `yujianwo` | I1 | MinIO access key |
| `MINIO_SECRET_KEY` | `yujianwo123` | I1 | MinIO secret key |
