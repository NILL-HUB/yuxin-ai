...
def register_routes(quart_app):
    ...
    # =====================================================
    # public_workflow_handler
    # =====================================================
    @quart_app.get("/public/workflows")
    async def public_workflow_get_public_workflows_with_page():
        from app.http import asgi_app as a
        ...
        workflows, paginator = await a._to_thread(
            a._get_service(PublicWorkflowService).get_public_workflows_with_page,
            req,
            account,
        )
        return a._ok({"list": workflows, "paginator": a.asdict(paginator)})

    @quart_app.get("/public/workflows/<uuid:workflow_id>")
    async def public_workflow_get_public_workflow_detail(workflow_id):
        from app.http import asgi_app as a

        from internal.service.public_workflow_service import PublicWorkflowService

        account = None
        workflow_detail = await a._to_thread(
            a._get_service(PublicWorkflowService).get_public_workflow_detail,
            workflow_id,
            account,
        )
        return a._ok(workflow_detail)

    @quart_app.get("/public/workflows/<uuid:workflow_id>/draft-graph")
    async def public_workflow_get_public_workflow_draft_graph(workflow_id):
        from app.http import asgi_app as a

        from internal.service.public_workflow_service import PublicWorkflowService

        graph = await a._to_thread(
            a._get_service(PublicWorkflowService).get_public_workflow_draft_graph,
            workflow_id,
        )
        return a._ok(graph)

    @quart_app.post("/workflows/<uuid:workflow_id>/share-to-square")
    async def public_workflow_share_workflow_to_square(workflow_id):
        from app.http import asgi_app as a
        ...
        tags = tags if tags else None
        await a._to_thread(
            a._get_service(PublicWorkflowService).share_workflow_to_square,
            workflow_id,
            tags,
            account,
        )
        return a._ok_msg("工作流已共享到广场")

    @quart_app.post("/workflows/<uuid:workflow_id>/unshare-from-square")
    async def public_workflow_unshare_workflow_from_square(workflow_id):
        from app.http import asgi_app as a
        ...
        await a._to_thread(
            a._get_service(PublicWorkflowService).unshare_workflow_from_square,
            workflow_id,
            account,
        )
        return a._ok_msg("工作流已从广场取消共享")

    @quart_app.post("/public/workflows/<uuid:workflow_id>/fork")
    async def public_workflow_fork_public_workflow(workflow_id):
        from app.http import asgi_app as a
        ...
        from internal.schema.public_workflow_schema import ForkWorkflowResp
        from internal.service.public_workflow_service import PublicWorkflowService

        workflow = await a._to_thread(
            a._get_service(PublicWorkflowService).fork_public_workflow,
            workflow_id,
            account,
        )
        return a._ok(ForkWorkflowResp().dump({"id": str(workflow.id), "name": workflow.name}))
