...
class GetAdminAgentPoolConfigsReq(Form):
...
class CreateAdminAgentPoolConfigReq(Form):
...
class UpdateAdminAgentPoolConfigReq(Form):
    enabled = StringField("enabled", validators=[Optional(), AnyOf(ENABLED_VALUES)])
    metadata_ = DictField("metadata_", default=None)


class SetAdminAgentPoolConfigStatusReq(Form):
