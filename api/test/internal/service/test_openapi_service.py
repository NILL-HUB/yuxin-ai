...
def _build_service() -> OpenAPIService:
    return OpenAPIService(
        ...
        app_runtime_service=SimpleNamespace(),
...
class TestOpenAPIService:
    ...
    def test_chat_should_stream_and_aggregate_agent_message_chunks(self, monkeypatch):
        ...
        app_config = {
            ...
            "knowledge_base_ids": [str(kb_dataset_id)],
        ...
        service.retrieval_service = SimpleNamespace(
            create_knowledge_retrieval_tool=lambda **kwargs: retrieval_call.update(kwargs) or "dataset-tool"
        )
        ...
        assert [str(kb_id) for kb_id in retrieval_call["knowledge_base_ids"]] == [str(kb_dataset_id)]
        assert retrieval_call["account_id"] == account.id
        assert retrieval_call["retrieval_strategy"] == "semantic"
        assert retrieval_call["k"] == 2
