...
def _build_service(llm_raises=False):
    service = TaskClassifierService.__new__(TaskClassifierService)
    service.language_model_service = MagicMock()
    if llm_raises:
        service.language_model_service.get_feature_model.side_effect = RuntimeError(
            "timeout"
        )
    return service
...
class TestTaskClassifierLLMDriven:
    ...
    def test_llm_deep_thinking_should_route_to_deep_thinking(self, mock_probe):
        ...
        assert decision.recommended_model_tier == "3"

    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_llm_vertical_agent_should_route_to_single_agent(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
    ...
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_llm_general_qa_should_route_to_direct_answer(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
            intent="general_qa",
    ...
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_llm_needs_deep_thinking_flag_overrides_intent(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
            intent="tool_task",
            execution_mode="SINGLE_AGENT_WITH_TOOLS",
            needs_deep_thinking=True,
    ...
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_llm_invalid_intent_falls_back_to_general_qa(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
...
class TestTaskClassifierConfidence:
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_confidence_should_be_recorded_in_reason(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
            intent="general_qa",
            execution_mode="DIRECT_ANSWER",
            confidence=0.92,
    ...
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_confidence_above_one_should_be_clamped(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
            intent="general_qa",
            execution_mode="DIRECT_ANSWER",
            confidence=1.5,
    ...
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_confidence_below_zero_should_be_clamped(self, mock_probe):
        mock_probe.return_value = TaskClassificationResult(
            intent="general_qa",
            execution_mode="DIRECT_ANSWER",
            confidence=-0.3,
...
class TestTaskClassifierKeywordFirst:
    ...
    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_keyword_miss_should_fallback_to_llm(self, mock_probe):
        ...
        assert decision.intent == "deep_thinking_task"
        # 关键词未命中，LLM 应被调用
        service.language_model_service.get_feature_model.assert_called_once()

    @patch.object(LLMActivityProbe, "invoke_structured_with_probe")
    def test_general_qa_keyword_miss_llm_returns_general_should_not_call_llm_again(
        self, mock_probe
    ):
