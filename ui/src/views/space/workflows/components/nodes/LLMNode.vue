...
<template>
  ...
  >
    ...
    <div class="flex flex-col items-start bg-gray-100 rounded-lg p-3 min-w-0">
      ...
      <div class="flex items-center gap-2 mb-2 text-gray-700">
        <icon-caret-down />
        <div class="text-xs font-semibold">{{ t('workflowEditor.prompt.title') }}</div>
      </div>
      <!-- 内容 -->
      <div class="w-full min-w-0 text-xs text-gray-700 leading-5 line-clamp-3 break-words overflow-hidden">
        {{ props.data?.prompt ?? '-' }}
      </div>
    </div>
    <!-- 输出变量列表 -->
