...
<template>
  <div
    :class="[
      'flex w-[360px] flex-col gap-3 overflow-hidden rounded-2xl border-[2px] bg-white p-3 shadow-sm hover:shadow-md',
  ...
  >
    ...
    <div class="flex flex-col items-start bg-gray-100 rounded-lg p-3 min-w-0">
      ...
      <div class="flex items-center gap-2 mb-2 text-gray-700">
        <icon-caret-down />
        <div class="text-xs font-semibold">{{ t('workflowEditor.templateTransform.title') }}</div>
      </div>
      <!-- 提示词内容 -->
      <div class="w-full text-xs text-gray-700 line-clamp-3 break-words overflow-hidden">
        {{ props.data?.template ?? '-' }}
      </div>
    </div>
    <!-- 输出变量列表 -->
